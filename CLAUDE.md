...
- **Config placement — env vs DB (classify every new config knob this way before adding it).** *Bootstrap/wiring* config — anything required to **start** a service or **connect components**, plus secrets — may live in env / Kubernetes config: DSNs, broker URLs, listen/health/metrics addresses, TLS cert+key paths & AMQP/PG servernames, the envelope-encryption KEK and other secrets, service-discovery URLs (e.g. delivery URL), body-store backend selection, OTLP endpoint, first-run bootstrap admin creds, dev start-mode toggles (insecure-auth). *Operational settings* — anything that **changes regularly, can differ per deployment, or is operator-tuned** — **must** be a row in the `settings` table (or the appropriate config table), set via the management plane (GUI / Admin API) and read by the planes from Postgres with `config.changed` hot-reload, **never an env var**: server identity/EHLO `hostname`, `alert_from`, `direct_helo`, `deliverability_interval_hours`, `max_message_bytes`, `retention_days`, and the like. (Resource-sizing knobs that must be set at process start and pair with pod resources — e.g. `NOVAMAIL_DELIVERY_CONCURRENCY` — stay env.)
